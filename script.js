...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("popupForm.html")
    ...
    .then((data) => {
      ...
      // ⭐ Universal Cooldown Check (shared across all pages)
      function checkCooldown() {
        const last = localStorage.getItem("lastSubmitTime");
        if (!last) return false;
        const diff = (Date.now() - parseInt(last)) / 60000;
        return diff < 20; // block if less than 20 minutes
      }
      ...
      async function submitLead() {
        ...
        try {
          ...
          if (msg.success) {
            ...
            // ⭐ Save cooldown time after successful submission
